        ...
        <div class="cartcontain" id="cart">
          ...
          <?php
            ...
            while($row = mysqli_fetch_array($result)){
              ...
              <a class='remove' href='remove.php?price=$price&orderid=$orderid'>
              REMOVE
              </a>
              </li>";
              echo $list;
  ...
  <main>
    <div class="wrapper">
      <ul class="grid">
        ...
        include('connect.php');
        $userid = $_SESSION['userid'];
        $query = "select orders.ID, orders.PRICE, orders.PICTURE, cars.MODEL from orders, cars where BUYER_ID = $userid AND STATE = '1' AND orders.MODEL_ID = cars.ID ORDER BY orders.ID";
        $result = mysqli_query($conn,$query);
        if($result){
        while($row = mysqli_fetch_array($result)){
          $orderid = $row["ID"];
          ...
          $carmodel = $row["MODEL"];
          $list =
          "<li class='item' id='$orderid' style='background:green;'>
          <a href='#' class='edit'>
            <div class='image' style=' background: url(".htmlspecialchars($orderpic).");background-size:cover;background-position:center;'>
            </div>
